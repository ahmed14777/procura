...
export async function POST(request: Request) {
  ...
  if (!password || password.length > 200 || !passwordMatches(password, process.env.STAFF_ACCESS_PASSWORD!)) {
    const failures = await recordFailure(identifier);
    const remaining = Math.max(0, MAX_ATTEMPTS - failures);
    return NextResponse.json(
      { error: remaining ? `Password errata. Tentativi rimasti: ${remaining}.` : "Troppi tentativi. Riprova tra 15 minuti." },
